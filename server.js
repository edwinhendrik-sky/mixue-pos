...
// 8. Endpoint Proses Absensi (Clock In / Clock Out) - Ditingkatkan Validasi & Stabilitasnya
app.post('/api/attendance', (req, res) => {
    let { employee_id, shift_id, store_id, type, selfie } = req.body;
    const today = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Jakarta' });
    const nowTime = new Date().toLocaleTimeString('it-IT', { timeZone: 'Asia/Jakarta' });

    // Pastikan employee_id dicari berdasarkan primary key (id) atau string employee_id
    db.get("SELECT * FROM employees WHERE id = ? OR employee_id = ?", [employee_id, employee_id], (err, empRow) => {
        ...
        db.get("SELECT store_name FROM stores WHERE id = ?", [store_id || empRow.store_id], (err, storeRow) => {
            ...
            db.get("SELECT shift_name, time_range FROM shifts WHERE id = ?", [shift_id], (err, shiftRow) => {
                ...
                // Cek apakah sudah ada record absensi untuk karyawan ini pada tanggal hari ini
                db.get("SELECT * FROM attendance WHERE employee_id = ? AND date = ?", [empRow.id, today], (err, existing) => {
                    if (type === 'in') {
                        if (existing && existing.clock_in) {
                            return res.json({ success: false, message: `⚠️ Gagal! Anda sudah melakukan Clock In hari ini pada pukul ${existing.clock_in}.` });
                        }
...
// 9. Endpoint Ambil Rekap Absensi (Di-JOIN otomatis dengan tabel employees agar ID String & Nama selalu sinkron)
app.get('/api/attendance', (req, res) => {
    const query = `
        SELECT a.id, e.employee_id as employee_id, e.name as employee_name, 
               COALESCE(s.store_name, a.store_name) as store_name, 
               a.shift_name, a.date, a.clock_in, a.clock_out, a.selfie, e.job_position
        FROM attendance a
